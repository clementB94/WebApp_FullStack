...
def get_rating(db: Session, movie_id: str, username: str):
    return db.get(models.Rating, (username,movie_id))
    
def get_ratings_by_user(db: Session, username: str):
    print(type(username), username)
    return db.query(models.Rating).filter_by(username=username).all()
...
def update_rating(db: Session, rating:schemas.RatingCreate):
    db_rating = db.get(models.Rating, (rating.username,rating.movie_id))
    # db.merge(db_rating)
...
def get_comments_by_user(db: Session, username: str):
    print(type(username), username)
    return db.query(models.Comment).filter_by(username=username).all()
